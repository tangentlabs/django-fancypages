...
class PageTreeMixin(object):
    ...
    def _create_grouped_pages(self):
        ...
        self.ungrouped_page = FancyPage.add_root(
            name='Ungrouped Page',
            **page_defaults
        )

        self.first_primnav_page = FancyPage.add_root(
            name="First primary navigation page",
            **page_defaults
        )
        self.first_primnav_page.groups.add(self.primary_nav_group)

        self.second_primnav_page = FancyPage.add_root(
            name="Second primary navigation page",
            **page_defaults
        )
        self.second_primnav_page.groups.add(self.primary_nav_group)

        self.footer_page = FancyPage.add_root(
            name="Footer page",
            **page_defaults
        )
        self.footer_page.groups.add(self.footer_group)

        self.both_groups_page = FancyPage.add_root(
            name="Both groups page",
            **page_defaults
        )
...
class TestPageTreeTag(PageTreeMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.footer_child = self.footer_page.add_child(name="Footer child")
    ...
    def test_returns_page_tree_without_primnav_child_with_depth_two(self):
        with self.assertNumQueries(1):
            page_tree = sm_tags.get_page_tree(
                group=self.primary_nav_group.slug,
                depth=2
            )
