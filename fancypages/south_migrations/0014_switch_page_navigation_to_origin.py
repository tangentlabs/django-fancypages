...
# because of the frozen ORM we don't have access to the attributes
# on the PageNavigationBlock model and need to re-define it here just for
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # We only have to update the relative blocks here because they have
        # to get different origin settings from the default. The rest is
        # already correct.
        relative_blocks = orm.PageNavigationBlock.objects.filter(
            is_relative=True)
        relative_blocks.update(origin=RELATIVE_FROM_CHILDREN)

    def backwards(self, orm):
        "Write your backwards methods here."
        relative_blocks = orm.PageNavigationBlock.objects.filter(
            origin=RELATIVE_FROM_CHILDREN)
        relative_blocks.update(is_relative=True)
